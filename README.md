...
The first `apply` in a fresh working directory may need to be run twice. `terraform-aws-modules/lambda/aws` computes the function's source code hash from the packaged archive, and on that first run the archive does not exist yet when the hash is computed, so plan and apply can disagree about it. This is upstream module behavior, not specific to this module. If it happens, just run `apply` again. `lambda_ignore_source_code_hash` works around it if it bothers you, at the cost of the function no longer redeploying automatically when its source changes, so it defaults to `false`.
...
| <a name="input_lambda_ignore_source_code_hash"></a> [lambda\_ignore\_source\_code\_hash](#input\_lambda\_ignore\_source\_code\_hash) | Passed straight through to terraform-aws-modules/lambda/aws. Set to true only as a workaround for the first apply in a fresh working directory needing to run twice, where the deployment archive does not exist yet when the module computes its source code hash. Leave false otherwise: true stops the function redeploying automatically when only its source changes. | `bool` | `false` | no |
